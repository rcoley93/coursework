...
class bigint {
   ...
   private:
      ...
      using unumber = vector<unsigned char>;
      friend quot_rem divide (const bigint&, const bigint&);
      friend bigint doAdd(const bigint& left, 
                                       const bigint& right,bool neg);
      friend bigint doSub(const bigint& left, 
                                       const bigint& right,bool neg);
      friend bool isNeg(const bigint& pos, const bigint& neg);
      friend void divide_by_2 (bigint& unumber_value);
      friend void multiply_by_2 (bigint& unumber_value);
   public:
